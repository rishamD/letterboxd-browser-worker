...
export async function scrapePage(username, page = 1) {
    ...
    try {
        ...
        if (res && res.status() === 403) {
            console.error("🚨 403 Forbidden — flagging for context refresh");
        ...
        await pageObj.waitForSelector('[data-component-class="LazyPoster"]', {
            timeout: 15000,
        });
        ...
        const data = await pageObj.evaluate(({ isFirstPage }) => {
            const results = [];
            document
                .querySelectorAll('[data-component-class="LazyPoster"]')
                .forEach((poster) => {
                    ...
                    if (
                        ...
                        const ratingSpan =
                            viewingData.querySelector(".rating");
                        if (ratingSpan) {
                            const ratedClass = [...ratingSpan.classList].find(
                                (c) => c.includes("rated-")
                            );
                            if (ratedClass) {
                                const score = ratedClass.match(/\d+/);
                                if (score)
                                    rating = parseInt(score[0], 10) / 2;
                            }
                        }
                    }
            ...
            if (isFirstPage) {
                const lastLink =
                    document.querySelector(
                        ".paginate-pages li:last-child a"
                    ) ||
                    document.querySelector(
                        ".paginate-pages a:last-of-type"
                    );
                if (lastLink) {
